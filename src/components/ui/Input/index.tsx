...
const Input: React.FC<IInput> = ({ fieldError, errorMessages, className, register, ...props }) => {
    const { deduceFormFieldErrors, isFormFieldValid } = useFormUtils()
    const fieldErrorLite = fieldError ? fieldError : null
    const reducedErrorMessages = deduceFormFieldErrors(errorMessages, fieldErrorLite)
            ...
            <div className="w-full relative mt-2 flex justify-start items-center rounded-md shadow-sm border ">
                ...
                <input
                    ...
                    className={clsx(
                        ...
                        props?.startIcon ? 'rounded-r-md' : props?.endIcon ? 'rounded-l-md' : 'rounded-md'
                    )}
                />
                {props?.endIcon && (
