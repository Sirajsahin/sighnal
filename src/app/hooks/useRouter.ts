import { IRouteChildren, IRouteDefined, IRoutesDefined, RouteKeys } from '../../api_framework/api_modals/routersDefine'
import { NAVIGATION_ROUTES, ROUTES, CAMPAIGNPAGESUBROUTE } from '../routes/routes'

export const useRouter = () => {
    const expandedSearch = (router: IRoutesDefined, route: RouteKeys): IRouteDefined | null => {
        const routeDetails: string | undefined = Object.keys(router).find((r) => r === route)
        if (routeDetails) {
            return router[route]
        } else {
            return null
    ...
    const getRouteKey = (route: RouteKeys, key: keyof IRouteDefined): string => {
        if (key === 'children') {
            throw new Error('Please use getRouteChildrens(route) method to get children')
        }
